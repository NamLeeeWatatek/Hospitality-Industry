...
import { ColorPallet } from "@spiffy_theme_backend_ent/js/color_pallet";
...
patch(NavBar.prototype, {
    async setup(parent, menuData) {
        ...
        $(document).off("click", ".theme_selector a").on("click", ".theme_selector a", function (ev) { self._openConfigModal(ev) })
        $(document).off('click', '#dark_mod').on('click', '#dark_mod', function (ev) { self._ChangeThemeModeCLicked(ev) });
        ...
        $(document).off('click', '.o_app_drawer a').on('click', '.o_app_drawer a', this._OpenAppdrawer.bind(this));
        ...
        $(document).off("click", ".header_to_do_list .to_do_list").on("click", ".header_to_do_list .to_do_list", function (ev) { self._openToDoList(ev) });
        ...
        await rpc('/get/irmenu/icondata', {
            ...
            $.each(menu_data, function (key, value) {
                ...
                if (!horizontalMenuContainer.data('menu-rendered')) {
                    const horizontalMenuTemplate = $(renderToElement("spiffy_theme_backend_ent.AppMenuGroup", {
                ...
                if ($('body').hasClass('top_menu_horizontal')) {
                    ...
                    if (!verticalMenuContainer.data('menu-rendered')) {
                        const verticalMenuTemplate = $(renderToElement("spiffy_theme_backend_ent.AllAppsMenus", {
                            group_info: value.spiffy_app_group,
                            menu_info: menu_data,
                            getMenuItemHref: self.getMenuItemHref,
                ...
                if (current_record.use_icon) {
                    ...
                    if (current_record.icon_img) {
                        ...
                        var icon_data = current_record.web_icon.split('/icon.')
                        var icon_parts = current_record.web_icon.split(',');

                        if (icon_data[1] == 'svg') {
                            var web_svg_icon = current_record.web_icon.replace(',', '/')
                            var icon_image = "<img class='img img-fluid' src='" + web_svg_icon + "' />"
                        } else if (icon_parts.length > 1 && icon_parts[0].startsWith('fa')) {
                            var icon_class = icon_parts[0].trim();
                            var icon_color = icon_parts[1] ? icon_parts[1].trim() : '#000';
                            var icon_image = "<span class='" + icon_class + " group_app_icon' style='color: " + icon_color + " !important;'></span>";
                        } else {
                            var icon_image = "<img class='img img-fluid' src='data:image/" + icon_data[1] + ";base64," + current_record.web_icon_data + "' />"
                        }
                    } else {
    ...
    addconfiguratorclass: function () {
        rpc('/get/model/record').then(function (rec) {
            ...
            if (rec.record_dict[0].font_size) {
                loadCSS(`/spiffy_theme_backend_ent/static/src/scss/font_sizes/${rec.record_dict[0].font_size}.css`);
            }
            ...
            if (fontLinks && fontLinks.google_font_links_ids && fontLinks.google_font_links_ids.length > 0) {
                fontLinks.google_font_links_ids.forEach(link => {
                    const fontName = link.name.replace(/\s+/g, "+");
                    const font_url = `<link href="https://fonts.googleapis.com/css2?family=${fontName}" rel="stylesheet" type="text/css">`;
                    $("head").append(font_url);
                    $('body').css('font-family', `'${link.name}'`);
                    $('#fontExistsMsg').hide();
                    $('#fontLimitMsg').hide();
                });
            } else {
                const rubikLink = `<link href="https://fonts.googleapis.com/css2?family=Rubik" rel="stylesheet" type="text/css">`;
                $("head").append(rubikLink);
                $('body').css('font-family', `'Rubik'`);
            }
            ...
            if (rec.record_dict[0].apply_light_bg_img) {
                if (rec.record_dict[0].light_bg_image) {
                    ...
                    $(".o_home_menu").attr("style", "background-image: url('/web/image/backend.config/"+rec.record_dict[0].id+"/light_bg_image') !important");
                }
            }
            ...
            if (rec.record_dict[0].top_menu_position == "top_menu_vertical_mini_2") {
                if (rec.record_dict[0].top_menu_bg_vertical_mini_2 == "top_menu_vertical_bg1") {
                    if (rec.record_dict[0].vertical_mini_bg_image_one) {
                        ...
                        $(".o_main_navbar").attr("style", "background: linear-gradient(to bottom, rgba(27, 27, 27, 1) 0%, rgba(27, 27, 27, 0.7) 60%, rgba(27, 27, 27, 0) 100%), url('/spiffy_theme_backend_ent/static/description/top-menu-v2-bg1.jpg') !important; background-size: cover !important; background-position: center !important;");
                        
                        $(".top_menu_vertical_mini_2.top_menu_vertical_mini_mobile .o_main_navbar").attr("style", "background: linear-gradient(to bottom, rgba(27, 27, 27, 1) 0%, rgba(27, 27, 27, 0.7) 60%, rgba(27, 27, 27, 0) 100%), url('/spiffy_theme_backend_ent/static/description/top-menu-v2-bg1.jpg') !important; background-size: cover !important; background-position: center !important;");
                    }
                }
                ...
                if (rec.record_dict[0].top_menu_bg_vertical_mini_2 == "top_menu_vertical_bg2") {
                    if (rec.record_dict[0].vertical_mini_bg_image_two) {
                        ...
                        $(".o_main_navbar").attr("style", "background: linear-gradient(to bottom, rgba(27, 27, 27, 1) 0%, rgba(27, 27, 27, 0.7) 60%, rgba(27, 27, 27, 0) 100%), url('/spiffy_theme_backend_ent/static/description/top-menu-v2-bg2.jpg') !important; background-size: cover !important; background-position: center !important;");
                        
                        $(".top_menu_vertical_mini_2.top_menu_vertical_mini_mobile .o_main_navbar").attr("style", "background: linear-gradient(to bottom, rgba(27, 27, 27, 1) 0%, rgba(27, 27, 27, 0.7) 60%, rgba(27, 27, 27, 0) 100%), url('/spiffy_theme_backend_ent/static/description/top-menu-v2-bg2.jpg') !important; background-size: cover !important; background-position: center !important;");
                    }
                }
                ...
                if (rec.record_dict[0].top_menu_bg_vertical_mini_2 == "top_menu_vertical_bg3") {
                    if (rec.record_dict[0].vertical_mini_bg_image_three) {
                        ...
                        $(".o_main_navbar").attr("style", "background: linear-gradient(to bottom, rgba(27, 27, 27, 1) 0%, rgba(27, 27, 27, 0.7) 60%, rgba(27, 27, 27, 0) 100%), url('/spiffy_theme_backend_ent/static/description/top-menu-v2-bg3.jpg') !important; background-size: cover !important; background-position: center !important;");
                        
                        $(".top_menu_vertical_mini_2.top_menu_vertical_mini_mobile .o_main_navbar").attr("style", "background: linear-gradient(to bottom, rgba(27, 27, 27, 1) 0%, rgba(27, 27, 27, 0.7) 60%, rgba(27, 27, 27, 0) 100%), url('/spiffy_theme_backend_ent/static/description/top-menu-v2-bg3.jpg') !important; background-size: cover !important; background-position: center !important;");
                    }
                }
                ...
                if (rec.record_dict[0].top_menu_bg_vertical_mini_2 == "top_menu_vertical_bg4") {
                    if (rec.record_dict[0].vertical_mini_bg_image_four) {
                        ...
                        $(".o_main_navbar").attr("style", "background: linear-gradient(to bottom, rgba(27, 27, 27, 1) 0%, rgba(27, 27, 27, 0.7) 60%, rgba(27, 27, 27, 0) 100%), url('/spiffy_theme_backend_ent/static/description/top-menu-v2-bg4.jpg') !important; background-size: cover !important; background-position: center !important;");
                        
                        $(".top_menu_vertical_mini_2.top_menu_vertical_mini_mobile .o_main_navbar").attr("style", "background: linear-gradient(to bottom, rgba(27, 27, 27, 1) 0%, rgba(27, 27, 27, 0.7) 60%, rgba(27, 27, 27, 0) 100%), url('/spiffy_theme_backend_ent/static/description/top-menu-v2-bg4.jpg') !important; background-size: cover !important; background-position: center !important;");
                    }
                }
            }
            ...
            if (rec.record_dict[0].menu_bg_image) {
                ...
                $(".new_systray").attr("style", "background-image: url('/spiffy_theme_backend_ent/static/description/header_vertical_mini.svg')");
                $(".top_menu_horizontal.top_menu_vertical_mini_mobile .o_main_navbar").attr("style", "background-image: url('/spiffy_theme_backend_ent/static/description/header_vertical_mini.svg')");
            }
    ...
    addFontManager: function () {
        $(document).on("click", "#addFontBtn", function () {
            const fontUrl = $("#fontInput").val()

            if (fontUrl) {
                
                // const fontNameMatch = fontUrl.match(/family=([^&]+)/);
                // const fontName = fontNameMatch ? decodeURIComponent(fontNameMatch[1].replace(/\+/g, " ")) : "Unknown Font";
                ...
                // let fontName = pathParts[specimenIndex + 1];
                ...
                if (fontName && fontName !== "Unknown Font") {
                    ...
                    // Preview the font
                    $('#previewContent').css('font-family', `'${fontName}', sans-serif`);
                    rpc('/add/google/font', { 
                        'name': fontName,
                        'url': fontUrl,
                    }).then(function (data) {
                        console.log("Returned font data:", data);
                        ...
                        if (data.status == 'success') {
                            ...
                            const NewFontFamilyAdd = $(renderToElement("spiffy_theme_backend_ent.AddNewFontFamily", {
                                fontId: data.id,
                                fontName: data.name,
                            }));  
        ...
        $(document).on("click", ".delete-font", function () {
            ...
            rpc('/delete/google/font', { 'id': fontId }).then(function (response) {
                if (response.status === "success") {
                    ...
                    console.log(`Font with ID ${fontId} deleted.`);
                } else {
    ...
    _searchPages: function () {
        ...
        if (searchvals_option === "menu") {
            ...
            this.$search_modal_results.empty().append(renderToFragment('spiffy_theme_backend_ent.MenuSearchResults', {
                results: results,
                widget: this,
            }));
        }
            ...
            options.each(function() {
                ...
                rpc('/get/records/data', {
                    ...
                    if (rec) {
                        ...
                        search_modal_results.empty().append(renderToFragment('spiffy_theme_backend_ent.RecordSearchResults', {

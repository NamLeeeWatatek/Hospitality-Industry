...
class BackendConfigration(http.Controller):
    ...
    def selected_pallet_data(self, **kw):
        ...
        font_family_vals = request.env['backend.config'].sudo().search(
                    [], order="id asc", limit=1)
        values = {}
        ...
        values.update({
            ...
            'font_family_vals': font_family_vals,
        })
    ...
    def get_record_data(self, **kw):
        ...
        font_dict = record_vals.read(['google_font_family'])[0]
        ...
        # Add to record_dict
        font_dict['google_font_links_ids'] = font_links
    ...
    def add_google_font(self, **post):
        ...
        if name and url:
            config_id = request.env['backend.config'].sudo().search([], limit=1)

            existing_font = request.env['google.font.family'].sudo().search([
                ('name', '=', name),
                ('config_id', '=', config_id.id)
            ], limit=1)

            if not existing_font:
                existing_google_fonts = request.env['google.font.family'].sudo().search([
                    ('config_id', '=', config_id.id)
                ])
                
                if len(existing_google_fonts) >= 5:
                    return {
                        "status": "limit_reached",
                        "message": "Maximum 5 fonts are allowed. Please delete one before adding a new font."
                    }

                # Set all other fonts to is_selected = False
                existing_google_fonts.write({'is_selected': False})

                # Create the new font with is_selected = True
                ...
                # If already exists, you can optionally also make it selected
                all_fonts = request.env['google.font.family'].sudo().search([
                    ('config_id', '=', config_id.id)
                ])
                all_fonts.write({'is_selected': False})

                existing_font.write({'is_selected': True})

                return {
                    "status": "duplicate",
                    "id": existing_font.id,
                    "name": existing_font.name,
                    "url": existing_font.url,
                    "config_id": config_id.id
                }
        else:
    ...
    def update_single_font_selection(self, font_id):
